...
var argv = require('yargs')
           .usage('$0 --protocol [protocol] --testName [test-name] --connectionString [connection-string]')
           .describe('connectionString', 'Device connection string')
           .describe('protocol', 'Protocol to use to connect the device to the hub')
           .describe('testName', 'Name of the test to run')
           .demandOption(['protocol', 'testName', 'connectionString'])
           .argv;
...
var client = Client.fromConnectionString(argv.connectionString, Protocol);
